...
<div class="card">
    ...
    <div class="text">Thank you, {{ $user->name }}! Your <strong style="color:#FF6B00;">{{ config("postpilot.plans.{$payment->plan}.label") }}</strong> is now active.</div>
    <div class="row"><span class="key">Plan</span><span class="val">{{ config("postpilot.plans.{$payment->plan}.label") }}</span></div>
    <div class="row"><span class="key">Amount Paid</span><span class="val" style="color:#FF6B00;">{{ $payment->amountInr() }}</span></div>
    <div class="row"><span class="key">Payment ID</span><span style="font-size:11px;color:#888;">{{ $payment->payment_id }}</span></div>
...
{{-- ============================================================ --}}
{{-- FILE: resources/views/emails/post-failed.blade.php          --}}
{{-- ============================================================ --}}
<!DOCTYPE html><html><head><meta charset="utf-8">
<style>
body{font-family:Arial,sans-serif;background:#0A0A0F;color:#E0E0F0;margin:0;padding:0}
...
<div class="card">
    <div class="title">[Admin] Post Publish Failed — #{{ $post->id }}</div>
    <div class="row"><span class="key">Post ID</span><span class="val">#{{ $post->id }}</span></div>
    <div class="row"><span class="key">User</span><span class="val">{{ $post->user->email ?? '—' }}</span></div>
    <div class="row"><span class="key">Retries</span><span class="val">{{ $post->retry_count }}/3</span></div>
    <div class="row"><span class="key">Scheduled</span><span class="val">{{ $post->scheduledAtIst() }}</span></div>
