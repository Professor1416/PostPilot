...
<head>
    ...
    <title>@yield('title', 'PostPilot') — AI Social Media Scheduler</title>
...
<header class="pp-header">
    <a href="{{ route('dashboard.calendar') }}" class="pp-logo">
    ...
    <nav class="pp-nav">
        ...
        <a href="{{ route('dashboard.queue') }}"    class="{{ request()->routeIs('dashboard.queue')    ? 'active' : '' }}">📋 Queue</a>
        <a href="{{ route('dashboard.accounts') }}" class="{{ request()->routeIs('dashboard.accounts') ? 'active' : '' }}">🔗 Accounts</a>
        <a href="{{ route('dashboard.pricing') }}"  class="{{ request()->routeIs('dashboard.pricing')  ? 'active' : '' }}">💳 Pricing</a>

        @auth
            <div class="pp-quota">
                <span class="pp-quota-num">{{ auth()->user()->isUnlimited() ? '∞' : auth()->user()->postsRemaining() }}</span>
                <span class="pp-quota-lbl">posts left</span>
            </div>

            <div style="display:flex;align-items:center;gap:8px;">
                @if(auth()->user()->photo_url)
                    <img src="{{ auth()->user()->photo_url }}" class="pp-avatar" alt="">
                @endif
                <div>
                    <div class="pp-user-name">{{ explode(' ', auth()->user()->name)[0] }}</div>
                    <div class="pp-plan-tag">{{ auth()->user()->plan }}</div>
                </div>
...
<div style="max-width:1280px;margin:0 auto;padding:0 28px;">
    @if(session('success'))
        <div class="pp-alert pp-alert-success" style="margin-top:16px;">✓ {{ session('success') }}</div>
    @endif
    @if(session('error'))
        <div class="pp-alert pp-alert-error" style="margin-top:16px;">✕ {{ session('error') }}</div>
    @endif
    @if(session('warning'))
        <div class="pp-alert pp-alert-warning" style="margin-top:16px;">⚠ {{ session('warning') }}</div>
    @endif
</div>
...
<footer style="display:flex;justify-content:space-between;align-items:center;padding:16px 28px;border-top:1px solid #0D0D1A;margin-top:40px;">
    ...
    <a href="{{ route('admin.login') }}" style="font-size:11px;color:#1A1A2A;text-decoration:none;">Admin</a>
</footer>
